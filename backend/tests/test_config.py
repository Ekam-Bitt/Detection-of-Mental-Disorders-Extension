...
class TestAppConfig:
    ...
    def test_from_env_uses_defaults(self):
        ...
        assert config.wellbeing_db_path.endswith("data/wellbeing.db")
    ...
    def test_from_env_respects_env_vars(self, monkeypatch):
        ...
        monkeypatch.setenv("WELLBEING_DB_PATH", "/tmp/test.db")
        config = AppConfig.from_env()
        assert config.model_path == "test-model"
        assert config.top_k == 5
        assert config.wellbeing_db_path == "/tmp/test.db"
    ...
    def test_to_flask_config_returns_dict(self):
        ...
        assert "WELLBEING_DB_PATH" in flask_config
